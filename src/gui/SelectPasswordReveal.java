...
/**
 * The Select Data File class. The class is responsible for displaying the
 * select data file screen in Revelation
 * 
 * @author Sean Hamlin
 * @version 1.0
 * @since 2.0
 */
public class SelectPasswordReveal {
	...
	private void addComponentsToPane(Container pane) {
		...
		CompoundBorder wholeBorder = new CompoundBorder(myLineBorder,
				innerBorder);
	...
	/**
	 * Application main method. Parameters: -Xms32m (minimum heap memory)
	 * -Xmx128m (maximum heap memory)
	 */
	public static void main(String args[]) {
		javax.swing.SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				new SelectPasswordReveal(null).CreateGUI();
			}
		});
	}
	 ...
	 * Default constructor
	 * 
	 * @param concealControl
	 *            a reference to concealControl
	 */
	public SelectPasswordReveal(ConcealControl concealControl) {
	...
	private static ImageIcon loadIcon(String aFileName) {
		...
		if (url != null)
			...
			System.out.println("Cannot locate image " + PATH + aFileName);
		return null;
	}
	...
	private class rightButtonAction implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			// if right arrow is enabled, password is either default, or
			...
			frame.dispose();
			EncodeStegoImage encodeStegoImage = new EncodeStegoImage(
					concealControl);
	...
	private class leftButtonAction implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			frame.dispose();
			SelectCoverFile selectCoverFile = new SelectCoverFile(
					concealControl);
	...
	private class helpButtonAction implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			new HelpDialog(frame, "concealStepThree.html",
					"Help with Conceal (Step 3)");
		}
	}
	...
	/**
	 * Takes care of the radio button being selected
	 * 
	 * @author Sean Hamlin
	 */
	...
	private class yourPasswordAction implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			...
			//check to see if passwords > 0 chars
			checkInput();
		}
	}
