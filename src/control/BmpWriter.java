...
public class BmpWriter extends Component {
	...
	// --- Revelation components that need updating
	...
	* @param myFrame a reference to the whole frame
	*/
	public BmpWriter(JProgressBar progress, JFrame myFrame) {
		this.progress = progress;
		this.myFrame = myFrame;
	}

	/**
	 * Saves the physical BMP file to the harddisk
	 * 
	 * @param parFilename
	 *            the File name for the new image
	 * @param parImage
	 ...
	 * writes the image.
	 * 
	 * @param parImage the image to be saved
	 * @param parWidth the width of the image
	 * @param parHeight the height of the image
	 * 
	 */
	...
	private void writeBitmap() {
		...
		rowCount = 1;
		padCount = 0;
		rowIndex = (size - biWidth) + 1; //last row starts at 90 not 89
		lastRowIndex = rowIndex;
		
		progress.setValue(0);
		progress.setMaximum(size);	//set to the number of pixels
		...
		try {
			for (j = 0; j <= size; j++) { // <==== Bug correction <=
				if (j % increment == 0) {
					progress.setValue(j);
				...
				if (rowCount == biWidth) {
					...
					rowCount = 1;
					rowIndex = (lastRowIndex - biWidth) - 1; // <==== Bug correction.  next row_index = 80 - 1 = 79
					lastRowIndex = rowIndex + 1; // <==== Bug correction 
				} else
					rowCount++;
				rowIndex++; //<==== Bug correction 79 + 1 = 80
			}
